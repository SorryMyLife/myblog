...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTableColumns() {
        let that=this
        httpget("aa/blog/blog-admin-table-columns/getColumns?t=6").then(
          (res)=>{
            if(res.code ==0){
      ...
      delRowFile(a, b) {
        ...
        this.$confirm('是否要删除[ ' + this.rowdata.llist.uid + ' ] ?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let fid=[]
          fid.push(this.rowdata.llist.uid+"----"+this.rowdata.llist.createTime)
          let parm={
            lids: fid.toString()
          }
          httppost("aa/blog/blog-login/admin/del",JSON.stringify(parm)).then(
            (res)=>{
              if(res.code ==0){
              this.$message({
                type: 'success',
                message: '删除' + this.rowdata.llist.uid + '成功!'
              });
               this.$router.go(0);
            }else{
              this.$message.error(res.msg);
      ...
      delfile() {
        ...
        for(var i=0;i<mup.length;i++){
          pp.push(mup[i].llist.uid+"----"+mup[i].llist.createTime)
        }
        let parm={
          lids: pp.toString()
        }
      ...
      getTableData() {
        ...
        httppost("aa/blog/blog-login/admin/list",JSON.stringify(parm)).then(
          (res)=>{
            if(res.code ==0){
      ...
      search() {
        ...
        httppost("aa/blog/blog-login/admin/search",JSON.stringify(parm)).then(
          (res)=>{
            if(res.code ==0){
